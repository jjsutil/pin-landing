...
I-009 (logo not navigating home) shipped, staging. Backlog: I-004 (static mode for
low-end hardware — one piece, the header's backdrop-filter cost, already extracted
and shipped standalone), I-012 (blog pagination doesn't scale past ~20 posts, dot-styled
`paginate()` once scheduled), I-013 (a scroll-driven vertical timeline view for the
blog, validated through a shared design Artifact — next up for implementation), I-014
(pinned/featured post ordering), and I-015 (a spike on view-count tracking — no
backend today, so this decides how before anything gets built). Full board:
[planning/BOARD.md](planning/BOARD.md) (generated by roadmap-board — never hand-edit).
<!-- BOARD-SUMMARY:END -->
